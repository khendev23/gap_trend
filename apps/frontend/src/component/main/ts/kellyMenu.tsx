...
export default function KellyMenu() {
    ...
    return (
        <section className="w-screen min-h-[60vh] bg-[#f9f9f9]">
            {/* 타이틀 */}
            <div className="flex items-center justify-between text-black px-6 md:px-12 pt-4">
            ...
            {/* 슬라이더 */}
            <div className="w-screen flex items-center justify-around mt-4 md:mt-6 px-2">
                {/* 이전 버튼 */}
                <button
                    type="button"
                    onClick={() => moveToSlide("prev")}
                    aria-label="이전 이미지"
                    className="w-12 h-12 text-2xl text-black/80 hover:text-black transition"
                >
                    {"<"}
                </button>
